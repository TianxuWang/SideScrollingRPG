...
.defines(function () {
        ...
        element: {
            ...
            draw: function() {
                for (var i = 0; i < ig.gui.elements.length; i++) {
                    ...
                    if((state == 'hover' && (ig.input.state('mouseLeft') || ig.input.pressed('mouseLeft'))) || (ig.input.pressed(element.keybind))) {
                        state = 'active';
                        if(ig.input.state('mouseLeft') && typeof ig.gui.elements[i].mouseDown == 'function')
                            ig.gui.elements[i].mouseDown.call(element);
                        if(ig.input.pressed('mouseLeft') || ig.input.pressed(element.keybind)) {
                            // Toggle (click)
                            if(element.toggle)
    ...
    ig.Game.inject({
        update: function() {
            ...
            // Initialize GUI
            if (ig.gui.initialized)
                return;
            else
                ig.gui.initialized = true;
        }
    });
});