...
.defines(function(){
    ...
    Weapon = Item.extend({
        ...
        init: function() {
            ...
            this._animSheet = new ig.AnimationSheet('media/items/item_icon.png', 16, 16);
        ...
        drawTooltip: function(x, y) {
            ...
            ig.game.assets.font_default.draw(this.price, tx+80, ty-2, ig.Font.ALIGN.RIGHT);
            var moneyIconPosX = tx+73-ig.game.assets.font_default.widthForString(this.price.toString());
            ig.game.assets.img_gold_icon.draw(moneyIconPosX, ty-2);
            ty += 8;
            ...
            if(ig.game.player.equipments.mainHand != this) {
                var currRating = ig.game.player.activeWeapon ? ig.game.player.getAverageDamage().value:0;
                var ratingDiff = (ig.game.player.getAverageDamage(this).value-currRating).round(1);
                var diffWidth = 0;
                if (ratingDiff > 0) {
