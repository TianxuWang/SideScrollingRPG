...
//            }
            ...
            if (this.target != null && this.target.state != this.target.State.DEAD){
                if (!(this.ai instanceof ChaseState))
                    this.ai = new ChaseState(this);
            }
            else if (!(this.ai instanceof PatrolState))
                this.ai = new PatrolState(this);
        ...
        makeRandDamage: function() {
            var crit = false;
            var randDamage = Math.floor((this._attack +
                Math.random()*(this.activeWeapon._maxDamage-this.activeWeapon._minDamage) + this.activeWeapon._minDamage)/2);
            if (Math.random() <= this.critHitChance) {
                crit = true;
                randDamage = Math.floor(randDamage*this.critHitDamage);
            }
            return {isCritical: crit, value: randDamage};
        }
    });
});