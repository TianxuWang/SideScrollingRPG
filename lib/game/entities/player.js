...
.defines ( function() {
    ...
    EntityPlayer = EntityCharacter.extend ({
        ...
        addStates: function() {
            ...
            this.stateMachine.state(this.State.IDLE, {
                ...
                update: function() {
                    self.idle();
                    weapon.idle();
                    //weapon.currentAnim.frame = self.currentAnim.frame;
                },
                exit: function() {
                }
            });
        ...
        hurt: function(fromLeft, damage) {

            this.state = this.State.HURT;
            this.hurtTimer.reset();

            var damageTaken = damage - this._defence;
            this._damageAccum += damageTaken;

            this.flip = fromLeft;

            this.bloodEmitter.setPos(this.pos.x + 4, this.pos.y + 4);
        ...
        tryActivateSpell: function(spell) {
            ...
            if (!spell.cooledDown()) {
                console.log(spell.name + ' is not cooled down');
                this.textEmitter.emit({
                    text: 'Not cool down',
                    color: '#0099FF',
                    gravityFactor: 0,
