...
.defines ( function() {
    ...
    EntityEnemy = EntityCharacter.extend ({
        ...
        init: function(x, y, settings) {
            ...
            // set initial state based on respawn or not
            this.stateMachine.currentState = settings.respawned ? this.State.INVINCIBLE : this.State.IDLE;
        ...
        addStates: function(_self) {
            ...
            this.stateMachine.state(this.State.INVINCIBLE, {
                enter: function() {
                    console.log('enemy begin invincible');
                    self.state = self.State.INVINCIBLE;
                    self.invincibleTimer.reset();
                },
                update: function() {
                    console.log('enemy invincible');
                    self.flashChain();
                },
                exit: function() {
                    console.log('enemy exit invincible');
        ...
        kill: function() {
            ...
            setTimeout(function(){
                ig.game.spawnEntity(EntityEnemy, self.origin.x, self.origin.y, {zIndex: 40, respawned: true});
        ...
        hurt: function() {
            // knock back
            var thresholdHealth = (1.5*(ig.game.player.attack-this.defence)/10)*ig.system.fps*ig.game.player.attackSpeed;

            var knockbackPower = this.health <= thresholdHealth ? 200 : 100;
            var knockupPower = this.health <= thresholdHealth ? 100 : 50;
